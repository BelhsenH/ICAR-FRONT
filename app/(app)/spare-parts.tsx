...
interface Part {
  _id: string;
  name: string;
  image: any; // Changed to any to support both local requires and URL strings
  price: string;
...
interface UserCar extends Vehicle {
...
const MechanicalPartsCatalogScreen = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Load parts by category - simplified mock data for now
              ...
              parts: [
                {
                  ...
                  name: 'Assemblage Moteur',
                  price: '2500 DT',
                  image: require('../../assets/images/engine-assembly.jpeg'),
                  availability: 'in-stock' as const,
                  stock: 5,
                  category: 'Moteur',
                  subCategory: 'Assemblage Moteur',
                  compatibleVehicles: [{ brand: 'BMW', model: 'X3', year: 2022 }]
                },
                {
                  _id: '2',
                  name: 'Bloc Moteur',
                  price: '1800 DT',
                  image: require('../../assets/images/engine-block.jpeg'),
                  availability: 'limited' as const,
                  stock: 2,
                  category: 'Moteur',
                  subCategory: 'Assemblage Moteur',
                  compatibleVehicles: [{ brand: 'Mercedes', model: 'C-Class', year: 2021 }]
                }
              ]
            }
          ]
              ...
              parts: [
                {
                  ...
                  name: 'Boîte de Vitesses',
                  price: '3200 DT',
                  image: require('../../assets/images/gearbox.jpeg'),
                  availability: 'limited' as const,
                  stock: 1,
      ...
      // Fetch real user vehicles
      if (token) {
        try {
          const userVehicles = await VehicleService.getUserVehicles(token);
          // Add a default image for vehicles that don't have one
          const carsWithImages = userVehicles.map(vehicle => ({
            ...vehicle,
  ...
  const handleOrder = async () => {
    if (!selectedPart || !selectedCar || !token) return;
    ...
    try {
      ...
      // Create parts request using the PartsService
                ...
                {userCars.length === 0 ? (
                  <View style={styles.noVehiclesContainer}>
                    ...
                    <Text style={styles.noVehiclesSubtext}>
                      Vous devez ajouter un véhicule pour pouvoir commander des pièces.
                    </Text>
                    <TouchableOpacity
                  ...
                  userCars.map((car) => (
                    ...
                    >
                      ...
                      <View style={styles.carInfo}>
                        <Text style={styles.carBrand}>{car.marque}</Text>
                        <Text style={styles.carModel}>{car.modele} ({new Date(car.datePremiereMiseEnCirculation).getFullYear()})</Text>
                        <Text style={styles.carPlate}>{car.numeroImmatriculation}</Text>
                      </View>
                  ...
                  <>
                    <View style={styles.summarySection}>
                      ...
                      <Text style={styles.summaryValue}>
                        {selectedCar.marque} {selectedCar.modele} ({new Date(selectedCar.datePremiereMiseEnCirculation).getFullYear()})
                      </Text>
                    </View>
                    <View style={styles.summarySection}>
                      <Text style={styles.summaryLabel}>Plaque:</Text>
                      <Text style={styles.summaryValue}>{selectedCar.numeroImmatriculation}</Text>
                    </View>
                  </>
                )}
                {orderNotes && (
            ...
            <View style={styles.stepContent}>
              <View style={styles.successContainer}>
                ...
                <Text style={styles.successMessage}>
                  Votre demande de pièce a été envoyée avec succès aux fournisseurs. Vous recevrez des notifications dès qu&apos;ils répondront à votre demande.
                </Text>
              </View>
            </View>
          );
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
      <View style={styles.container}>
        ...
        {/* Floating Action Button for Parts Request */}
        <TouchableOpacity
          style={styles.fab}
